...
namespace SkiExplorer.Controllers
{
    ...
    public class StazaController : ControllerBase
    {
        ...
        [HttpGet("PreuzmiStazu")]
        public async Task<IActionResult> PreuzmiStazu(string nazivStaze)
        {
            try
            {
                using (var session = _driver.AsyncSession())
                {
                    var result = await session.ExecuteReadAsync(async tx =>
                    {
                        var query = "MATCH (s:Staza {naziv: $naziv}) RETURN s";
                        var cursor = await tx.RunAsync(query, new { naziv = nazivStaze });
                        var nodes = new List<INode>();
